...
def simulated_annealing(data, gauss, initial_temp = 1500, cooling_rate = 0.3, max_iterations=250):
...
class EvolutionAlgorithm:
    ...
    def evolve(self):
        write_EA = []
        write_EA.append([instance + ', Pop_size: ' + str(ga.pop_size) + ', Gens: ' + str(ga.generations) + ', Cross_prob: ' + str(ga.cross_prob) 
                         + ', Mut_prob: ' + str(ga.mut_prob) + ', Tour: ' + str(ga.Tour)+ ', elite_ratio: ' + str(ga.elite_ratio) + ', Mutacja: change/swap' + ', Krzyzowanie: OX'])
        ...
        for i in range(self.generations):
            ...
            write_EA.append([int(i), best_solution(self.pop)[3], avg_solution(self.pop), worst_solution(self.pop)[3]])
            while len(new_pop) != self.pop_size:
                ...
                new_pop.append(individual)
            self.pop = new_pop
        write_EA.append([int(self.generations), best_solution(self.pop)[3], avg_solution(self.pop), worst_solution(self.pop)[3]])
        save_to_csv(os.path.join(current_dir, "ga_process.csv"), write_EA)
    ...
    def mutate(self, solution):
        cids = list(solution[0].keys())
        random.shuffle(cids)
        for cid in cids:
            for a, b, c, lecture in solution[0][cid]:
                neighbor_schedule = deepcopy(solution[0])
                day, period, room, lecture_num = solution[0][cid][lecture - 1]
                ...
                if roulette <= 1/3:
                    ...
                    neighbor_schedule_3[rand_cid][rand_lecture] = (swap_1_day, swap_1_period, swap_1_room, swap_2_lecture)
                if roulette <= 1/3:
                    neighbor_schedule = deepcopy(neighbor_schedule_1)
                elif roulette <= 2/3 and roulette > 1/3:
                    neighbor_schedule = deepcopy(neighbor_schedule_2)
                else:
                    neighbor_schedule = deepcopy(neighbor_schedule_3)
                neighbor_hard, neighbor_soft, neighbor_penalty = penalty(neighbor_schedule, data)
        return neighbor_schedule, neighbor_hard, neighbor_soft, neighbor_penalty
...
if __name__ == "__main__":
    ...
    for instance in instances:
        ...
        #save_html_timetable(os.path.join(current_dir,"wyniki", f"{os.path.splitext(instance)[0]}"), "sa_timetable.html", html)
        write.append([os.path.splitext(instance)[0], best_random[3], avg_solution(random_pop), worst_solution(random_pop)[3], None, avg_time(time_random_pop),
                      best_sa[3], avg_solution(sa_pop), worst_solution(sa_pop)[3], None, best_sa[1], avg_time(time_sa_pop),
                      best_ga[3], avg_solution(ga_solutions), worst_solution(ga_solutions)[3], None, best_ga[1], avg_time(time_ga_pop)])
    #std_solution(random_pop), std_solution(sa_pop), std_solution(ga_solutions)
